...
const ProductSchema = new mongoose.Schema({
  ...
  image: String, category: String, subcategory: String, brand: String,
  specs: mongoose.Schema.Types.Mixed, keyFeatures: [String],
  brochureLink: String, videoLink: String, isAvailableOnGeM: Boolean,
}, { timestamps: true });

const BlogSchema = new mongoose.Schema({
  id: String, title: String, content: String, author: String,
  date: String, published: Boolean, image: String, excerpt: String, tags: [String],
}, { timestamps: true });
...
const createModel = (id, title, subtitle, specs = {}, description = '', keyFeatures = []) => ({
  ...
  image: '/assets/shredders.png', category: 'Document Shredders', subcategory: 'Office',
  brand: title.toUpperCase().startsWith('AVANTI') ? 'AVANTI' : 'ANTIVA',
  specs, keyFeatures, brochureLink: '#', videoLink: '', isAvailableOnGeM: subtitle?.includes('GeM') ?? false,
});
const createAppModel = (id, title, subtitle, specs = {}, description = '', keyFeatures = []) => ({
  id, title, subtitle, description, image: '/assets/application-shredders.png',
  category: 'Special Application', subcategory: 'Industrial', brand: 'AVANTI',
  specs, keyFeatures, brochureLink: '#', videoLink: '',
});
const createLaminatorModel = (id, title, subtitle, specs = {}, description = '', keyFeatures = []) => ({
  id, title, subtitle, description, image: '/assets/binders-and-laminators.png',
  category: 'Laminators', subcategory: 'Office', brand: 'AVANTI',
  specs, keyFeatures, brochureLink: '#', videoLink: '',
});

const PRODUCTS = [
  { id: 'antiva-ps-225', title: 'ANTIVA PS 225', subtitle: 'DESK SIDE DOCUMENT SHREDDER', description: 'For high security shredding and continuous operation.', image: '/assets/shredders.png', category: 'Document Shredders', subcategory: 'Desk Side', brand: 'ANTIVA', keyFeatures: ['Auto Start & Stop','High-grade alloy steel cutters','Low noise design','All-metal construction','Overload protection with buzzer','30-minute continuous operation','Full spare parts supply'], specs: { 'Feed width (mm)': '225 mm','Shred size': 'Strip Cut','Shred Capacity (70 gsm A4)': '8 sheets','Voltage': '220V','Power': '190W','Speed': '2 m/min','Waste Volume': '18L','Weight': '17 Kg','Dimensions (HxWxD)': '575x335x420 mm','Motor Start/Off': 'Paper Sensor','Working cycle': 'Continuous' }, brochureLink: '#', videoLink: '', isAvailableOnGeM: false },
...
const BLOGS = [
  { id:'1', title:'Why Shredding is Important for Data Security', content:"<p>In today's digital age, physical document security is often overlooked. However, dumpster diving remains a common tactic for identity thieves. A cross-cut or micro-cut shredder is your first line of defence against data breaches that originate from discarded paperwork.</p><p>Government agencies require DIN 66399 compliant shredding. Corporate offices dealing with confidential employee records, financial statements, and client data should invest in at minimum a P-4 security-level shredder.</p>", author:'Admin', date:'2023-10-25', published:true, image:'https://placehold.co/800x400/0A7A45/ffffff?text=Data+Security', excerpt:"Physical document security is often overlooked in the digital age — here's why it shouldn't be.", tags:['data security','shredding','compliance'] },
  { id:'2', title:'Choosing the Right Shredder for Your Office', content:'<p>Strip-cut, cross-cut, or micro-cut? The security level you need depends on the sensitivity of the documents you handle. Strip-cut offers the least security, while micro-cut (P-5 and above) meets government-grade requirements.</p><p>Volume matters too — a desk-side shredder handles 8–12 sheets and suits individual use, while departmental units handle 30–100+ sheets for shared team environments.</p>', author:'Admin', date:'2023-11-12', published:true, image:'https://placehold.co/800x400/065A32/ffffff?text=Choosing+a+Shredder', excerpt:'Strip-cut, cross-cut, or micro-cut? A guide to selecting the right shredder for your needs.', tags:['buying guide','shredders','office equipment'] },
];

console.log('🗑  Clearing existing products and blogs...');
await Promise.all([Product.deleteMany({}), Blog.deleteMany({})]);
...
if (!existing) { await SiteSettings.create({}); console.log('⚙️  Default site settings created.'); }

console.log(`✅ Done! Seeded ${PRODUCTS.length} products and ${BLOGS.length} blogs.`);
